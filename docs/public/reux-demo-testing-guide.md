...
It also checks the Business Simulator API contract that the Reuben website uses: CORS preflight, template loading, simulation run, scenario comparison, scenario ranking, recommendation score breakdown, runner-up score gap metadata, saved-run reload/listing, missing-run handling, frontend-ready result metadata, and Reux source transparency.
