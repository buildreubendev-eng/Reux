...
- `simulation-ir` and `simulation-run` emit and execute the first prototype formula-based simulation forecast model with lightweight units, shared scheduled assumption changes, scenario-specific scheduled changes, scenario overrides, and final-period deltas.
