...
- `simulation-ir` and `simulation-run` emit and execute the first prototype formula-based simulation forecast model with lightweight units, shared scheduled assumption changes, scenario-specific scheduled changes, scenario overrides, final-period deltas, period-by-period deltas, and first-divergence reporting.
