...
Simulation IR records static assumptions, lightweight units, formulas, objectives, time-varying changes, scenarios, and forecast windows from `simulate` declarations. It is intentionally separate from Schema IR because simulations are product/domain behavior rather than durable database shape.
...
The current runner is a prototype formula forecast. It repeats declared assumptions over the forecast window, applies scheduled assumption changes, and emits formula results as period metrics with `metricUnits` when the formula is unit-compatible. Formula expressions support numeric literals, assumptions, earlier formulas, parentheses, and basic arithmetic. The compiler validates formula unit compatibility before run time, rejecting unsupported compound-unit math such as `USD * count` and incompatible additive math such as `USD + percent`. When scenarios are declared, the runner validates override units, evaluates each scenario's assumption overrides plus shared scheduled changes and scenario-specific scheduled changes, and emits comparison artifacts against baseline. Comparisons include final-period metric deltas, period-by-period deltas, units for comparable metrics, the first period where the scenario diverges from baseline, per-metric rankings, and explanation summaries. Rankings are objective-aware when a simulation declares `objective maximize metric` or `objective minimize metric`; metrics without objectives keep the neutral descending-delta order.

The TypeScript simulation contract emitter generates reusable run/result interfaces plus simulation-specific assumption, metric, scenario, explanation, summary, and run types. It also exports metadata constants with forecast windows, assumptions, metrics, objectives, and scenario names so web apps can wire UI controls without hand-copying source details. Generated helpers can format deltas, find rankings and explanations by metric, summarize comparisons, and list per-scenario metric deltas for frontend/backend consumers. The generated file also includes a `reuxSimulationCatalog`, `listReuxSimulationNames()`, and `findReuxSimulationMetadata(name)` so PLOS and business-simulation product apps can discover available models through a small typed API:
...
The scaffold discovers `enqueue Event { ... }` statements and `after commit hook(...)` calls from transaction functions. It generates placeholder `OutboxHandler` and `AfterCommitHandler` objects, configures graceful shutdown for `SIGINT`/`SIGTERM`, requeues stale processing claims using `REUX_WORKER_REQUEUE_STALE_SECONDS`, configures delayed retry and dead-letter behavior with `REUX_WORKER_MAX_ATTEMPTS` and `REUX_WORKER_RETRY_DELAY_SECONDS`, and runs `runOutboxWorker` against the configured PostgreSQL database. Placeholder after-commit handlers log `hook.resolvedArgs ?? hook.args` so copied handlers can opt into runtime argument resolution without losing source-level visibility.
