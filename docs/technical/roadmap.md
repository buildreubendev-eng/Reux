...
1. Transaction language depth: first slice complete with stronger expression validation, durable idempotency keys, explicit abort lowering, typed event payloads, require guards that reject unknown references before SQL lowering, and typed generated worker contracts; remaining work is richer multi-step control flow and deeper expression typing.
...
6. Editor tooling: first slice complete with CLI formatting, a local VS Code grammar, and CLI-backed file diagnostics for `.dl` and `.reux` files; remaining work is go-to-definition, completion, hover text, and editor-native formatting.
