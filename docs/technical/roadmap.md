...
- Schema declarations for modules, entities, enums, typed events, indexes, checks, generated IDs, references, transition rules, executable aggregate views, executable single-entity rules, bounded decimals, and currency codes.
...
8. Operating-model language layer: started with `view` and `rule` declarations plus IR and PostgreSQL lowering for count, sum, average, min, and max dashboard metrics, mark actions, durable notification lifecycle helpers, and duplicate-safe outbox notification actions; next work is richer typed view expressions, read-model API generation, escalation policies, and running rules as deterministic jobs or transaction guards.
...
- Demo readiness: roughly 100%. The commerce, logistics, and Business Simulator demos are public-UI ready, have health, deep, hosted, and CI-backed smoke checks, expose session-scoped queue health endpoints, show domain-scoped queue health in the public dashboard, include in-page tester guidance and clearer empty states, support PostgreSQL-backed saved simulation runs for result pages, validate saved-run creation/reload/listing in deep health checks, expose public API rate limiting and request counters, include maintenance cleanup for visitor schemas and saved runs, include optional monitor webhook alerts, and produce useful worker retry/dead-letter/stale-requeue logs. Remaining work is product expansion and production operations hardening rather than closing the demo-readiness bar.
