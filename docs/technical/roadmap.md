...
- PostgreSQL runtime helpers for migrations, compiled queries, transaction execution, seed resets, and outbox processing with delayed retries, dead-lettering, stale-claim recovery, and worker observability counters.
...
| Production monitoring | Started | Hosted health exists, `npm run demo:healthcheck -- <url>` validates the public service, and worker logs expose retry/dead-letter/stale-requeue counters. Alerting, uptime tracking, and DB maintenance are still manual. |
...
3. Production worker semantics: first slices complete with delayed retries, max-attempt dead-lettering, generated worker controls, stale claim recovery, per-iteration worker observability counters, and operational docs; remaining work is hosted worker dashboards and richer handler-level traces.
