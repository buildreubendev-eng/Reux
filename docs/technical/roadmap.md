...
2. Simulation language foundation: first slices complete with static `simulate` declarations, named domain dimensions, advisory PLOS/business domain pack reports, arithmetic formulas, lightweight unit-compatibility checks, shared scheduled changes, scenario-specific timelines, scenario overrides, maximize/minimize objectives, first-divergence reporting, period-level deltas, objective-aware metric rankings, explanation summaries, generated TypeScript contracts, typed comparison helpers, and a prototype runner; remaining work is production-grade simulation semantics and richer domain packs.
