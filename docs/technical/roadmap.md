...
6. Editor tooling: first slice complete with CLI formatting, a local VS Code grammar, CLI-backed file diagnostics, and CLI-backed document formatting for `.dl` and `.reux` files; remaining work is go-to-definition, completion, hover text, and a proper language-server process.
