...
| Production monitoring | Started | Hosted health exists, `npm run demo:healthcheck -- <url>` validates the public service, `npm run demo:healthcheck -- <url> --deep` validates queue stats endpoints, `outbox-stats` and demo API queue endpoints summarize queue health, and worker logs expose retry/dead-letter/stale-requeue counters. Alerting, uptime tracking, and DB maintenance are still manual. |
