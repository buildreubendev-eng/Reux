...
| Production monitoring | Started | Hosted health exists, `npm run demo:healthcheck -- <url>` validates the public service, `outbox-stats` and demo API queue endpoints summarize queue health, and worker logs expose retry/dead-letter/stale-requeue counters. Alerting, uptime tracking, and DB maintenance are still manual. |
...
- Demo readiness: roughly 94%. The commerce and logistics demos are public-UI ready, have a command-line hosted health check, expose session-scoped queue health endpoints, and now produce useful worker retry/dead-letter/stale-requeue logs; the remaining demo gap is mainly polish, alerting, uptime tracking, DB maintenance, and hosted redeployment validation.
