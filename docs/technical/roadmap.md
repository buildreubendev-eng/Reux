...
- Simulation declarations with static assumptions, lightweight units, formulas, maximize/minimize objectives, shared and scenario-specific time-varying changes, scenarios, forecast windows, Simulation IR, period-level comparison reports, objective-aware metric rankings, and a prototype formula forecast runner.
...
2. Simulation language foundation: first slices complete with static `simulate` declarations, arithmetic formulas, lightweight units, shared scheduled changes, scenario-specific timelines, scenario overrides, maximize/minimize objectives, first-divergence reporting, period-level deltas, objective-aware metric rankings, and a prototype runner; remaining work is richer dimensional analysis, explanation summaries, and generated TypeScript contracts.
