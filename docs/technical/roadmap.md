...
6. Editor tooling: first slice complete with CLI formatting, a local VS Code grammar, CLI-backed file diagnostics, CLI-backed document formatting, lightweight completions, hover text, and current-file go-to-definition for `.dl` and `.reux` files; remaining work is compiler-backed semantic completion, cross-file navigation, rename, precise diagnostic ranges, and a proper language-server process.
