...
- Simulation declarations with static assumptions, formulas, scenarios, forecast windows, Simulation IR, and a prototype formula forecast runner.
...
2. Simulation language foundation: first slices complete with static `simulate` declarations, arithmetic formulas, scenario overrides, and a prototype runner; remaining work is typed units, time-varying assumptions, richer comparison reports, and generated TypeScript contracts.
...
- Full completion: roughly 62%. The foundation is real, query expressiveness has moved forward, transaction functions now have the first serious safety layer, and Reux has its first simulation syntax with formulas and scenario comparison. The largest remaining gaps are production worker semantics, migration operations, editor tooling, packaging, richer control flow, deeper expression typing, typed units, and production-grade simulation semantics.
